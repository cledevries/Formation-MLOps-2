...
def predict_endpoint():
    ...
    received_data_df = pd.DataFrame(
        ...
        index=[0])
    data = prepare_features(data=received_data_df, training_mode=False)
    prediction = predict(features=data, model_path=MODEL_PATH)
    # raise NotImplementedError
    return jsonify(prediction.to_dict()["predictions"])
